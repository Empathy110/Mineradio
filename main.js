...
// ★ 性能优化: 移除 disable-background-timer-throttling / disable-renderer-backgrounding / disable-backgrounding-occluded-windows
// 这三个开关会阻止 Chromium 在后台时对渲染进程进行节流，导致后台放歌时 CPU/GPU 占用依然很高。
// 移除后，Chromium 会在窗口最小化/不可见时自动降低渲染频率，大幅减少 CPU/GPU 消耗。
// 同时保留 CalculateNativeWinOcclusion 让 Windows 原生窗口遮挡检测生效。
...
// 通过 CSS transition 实现，GPU 加速，CPU 开销极小
...
function showWithDrawerAnimation(win) {
  ...
  function animateDrawer() {
    ...
    const currentY = Math.round(startYPos + (endYPos - startYPos) * eased);
...
async function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // 配合 index.html 中的 render-deep-sleep 机制，后台放歌时 CPU/GPU 占用大幅降低。
  ...
  mainWindow.once('ready-to-show', () => {
    ...
    // 检测环境变量: MINERADIO_DISABLE_DRAWER_ANIM=1 可禁用
